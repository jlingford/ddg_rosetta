...
def generate_ticks_positions(values, config):
    ...
    if spacing is None:
        if int_type == "discrete":
            ...
            # rounded up
            spacing = int(np.ceil(np.linspace(bottom, top, steps, retstep=True)[1]))
        elif int_type == "continuous":
            # Default spacing is the one between two steps
...
def plot_total_heatmap(
    ...
    if saturation:
        ...
        final_df = df[df[state_col] == "ddg"][[pos_label_col, MUTR, tot_score_col]]
    ...
    # annotations
    h_config_heat, h_config_annot = get_items(h_config, ("heatmap", "annot"), {})
    ...
    generate_colorbar(mappable=ax.get_children()[0], ticks=c_ticks, config=c_config)

    # Set the x-axis
    set_axis(ax=ax, axis="x", ticklabels=x_ticklabels, config=x_config)

    # Set the y-axis
    set_axis(ax=ax, axis="y", ticks=y_ticks, ticklabels=y_ticklabels, config=y_config)
...
def plot_dg_swarmplot(df, config, out_file, out_config):
    ...
    # Set the x-axis
    set_axis(ax=ax, axis="x", config=x_config, ticks=x_ticks, ticklabels=x_ticklabels)
