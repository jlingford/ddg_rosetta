...
from .util import get_items, _recursive_traverse


############################# DATA LOADING ############################


def load_aggregated_data(in_file, saturation=False):
...
def generate_ticks_positions(values, config):
    ...
    if spacing is None:
        if int_type == "discrete":
            ...
            # rounded up
            spacing = int(
                np.ceil(np.linspace(bottom, top, steps, retstep=True)[1]))
        elif int_type == "continuous":
            # Default spacing is the one between two steps
            spacing = np.linspace(bottom, top, steps, retstep=True)[1]
    ...
    if ciz:
        ...
        absval = np.ceil(top) if top > bottom else np.floor(bottom)
...
def generate_heatmap_annotations(df, config):
    ...
    if config.get("annot"):
        ...
        def annot_func(x): return np.around(x, n_decimals)
...
def generate_barplot_annotations(ax, config, sum_pos, total, yticks):
...
def generate_axhline(ax, config, df):
    ...
    # Plot the line
    plt.axhline(y=y, xmax=xmax, **config)


def generate_colorbar(mappable, ticks, config):
    ...
    orient = (
        config["colorbar"].get("orientation")
        if config["colorbar"].get("orientation") is not None
        else "vertical"
    )
...
def generate_legend(ax, config):
    ...
    # Draw the legend
    ax.legend(
        handles=handles, labels=labels, bbox_transform=plt.gcf().transFigure, **config
    )
...
def set_axis(ax, axis, config, ticks=None, ticklabels=None):
    ...
    # If it is the y-axis
        ...
        # Set the tick labels
        ax.set_yticklabels(labels=ticklabels, **ticklabels_config)
    ...
    if config.get("tick_params"):
        ...
        ax.tick_params(axis=axis, **config["tick_params"])
...
def plot_total_heatmap(
    df, config, out_file, out_config, saturation=False, saturation_on=None
):
    """Plot either:
    ...
    if saturation:
        ...
        final_df = df[df[state_col] == "ddg"][[
            pos_label_col, MUTR, tot_score_col]]
        ...
        final_df = final_df.pivot(
            index=pos_label_col, columns=MUTR, values=tot_score_col
        ).transpose()
    ...
    # If the data are not from a saturation mutagenesis scan
        ...
        final_df = df[df[state_col] == "ddg"][[mut_label_col, tot_score_col]]
    ...
    # x- and y-axis
    h_config, c_config, nan_config, x_config, y_config = get_items(
        config, ("heatmap", "colorbar", "nancells", "xaxis", "yaxis"), {}
    )

    # Get the configuration for the heatmap grid and for the
    # annotations
    h_config_heat, h_config_annot = get_items(
        h_config, ("heatmap", "annot"), {})

    # Get the configuration for the interval to be represented
    # on the colorbar
    c_config_int = get_items(c_config, ("interval",), {})[0]

    # ----------------------------- Plot ------------------------------#

    # Get the colorbar ticks positions
    c_ticks = generate_ticks_positions(values=y_values, config=c_config_int)

    # Get maximum and minimum value from the interval
    vmin, vmax = c_ticks[0], c_ticks[-1]

    # Generate the heatmap annotations
    annots = generate_heatmap_annotations(df=final_df, config=h_config_annot)
    ...
    generate_mask_nancells(ax=ax, cells=nan_cells, config=nan_config)

    # Add the colorbar to the heatmap
    generate_colorbar(mappable=ax.get_children()[
                      0], ticks=c_ticks, config=c_config)

    # Set the x-axis
    set_axis(ax=ax, axis="x", ticklabels=x_ticklabels, config=x_config)

    # Set the y-axis
    set_axis(ax=ax, axis="y", ticks=y_ticks,
             ticklabels=y_ticklabels, config=y_config)
...
def plot_dg_swarmplot(df, config, out_file, out_config):
    ...
    # columns of interest
    df = df[[mut_label_col, tot_score_col, state_col, struct_num_col]]
    ...
    # the y-axis
    s_config, l_config, x_config, y_config = get_items(
        config, ("swarmplot", "legend", "xaxis", "yaxis"), {}
    )
    ...
    y_ticks = generate_ticks_positions(values=y_values, config=y_config_int)

    # Generate the swarmplot
    ax = sns.swarmplot(data=df, x=x, y=y, hue=hue, **s_config)

    # Add the legend
    generate_legend(ax=ax, config=l_config)

    # Set the x-axis
    set_axis(ax=ax, axis="x", config=x_config,
             ticks=x_ticks, ticklabels=x_ticklabels)

    # Set the y-axis
    set_axis(ax=ax, axis="y", config=y_config, ticks=y_ticks)
...
def plot_contributions_barplot(df, config, contributions, out_file, out_config):
    ...
    b_config, l_config, axh_config, x_config, y_config = get_items(
        config, ("barplot", "legend", "axhline", "xaxis", "yaxis"), {}
    )

    # Get the configuration for the bars and for the annotations
    # displayed over the bars
    b_config_bars, b_config_annot = get_items(b_config, ("bars", "annot"), {})
    ...
    with PdfPages(out_file) as pdf:
        ...
        for page in range(num_pages):
            ...
            sub_df_contr = sub_df[sub_df[state_col] == "ddg"][contributions]

            # Get the total scores for the current position
            sub_df_total = sub_df[sub_df[state_col] == "ddg"][tot_score_col]
            ...
            # Get the positions of the ticks on the y-axis
            y_ticks = generate_ticks_positions(
                values=y_values, config=y_config_int)

            # Generate text annotations with the total ΔΔG scores
            # that will appear on the top of the bars
            generate_barplot_annotations(
                ax=ax,
            ...
            # y coordinate 0.0
            generate_axhline(ax=ax, config=axh_config, df=sub_df_total)
            ...
            # Set the x-axis
            set_axis(
                ax=ax, axis="x", config=x_config, ticks=x_ticks, ticklabels=x_ticklabels
            )
